...
def database_update():
  ...
  print("[+]Updating database - Last update: " + database_last_date('database/local_vulnerable_files.xml'))
...
def display_vulnerable_component(name, version, file):
    list_id = []
    # Load json file
    json_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), "../database/" + file + ".json")
    ...
    if name in data.keys():
      ...
      if is_lower(version, data[name]['latest_version'], False):
        ...
        print("[+]The version is out of date, the latest version is %s" % data[name]['latest_version'])

      # Display the vulnerability if it's not patched version
      for vuln in data[name]['vulnerabilities']:
        if 'fixed_in' in vuln.keys() and (vuln['fixed_in'] is None or is_lower(version, vuln['fixed_in'], True)):

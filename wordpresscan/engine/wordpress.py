...
class Wordpress:
	...
	def random_agent(self):
		if self.agent != "random_agent":
			...
			user_agent_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), "../database/user-agents.txt")
	...
	def is_wordpress(self, nocheck):
		self.index = requests.get(self.url, headers={"User-Agent":self.agent}, verify=False, timeout=10)
		if nocheck == False:
			if "wp-" not in self.index.text:
				# print(critical("Not a WordPress !"))
				print("[!]Error! Not a WordPress website")
				exit()
	...
	# 				print("[!]Error! Website is not fully configured and currently in install mode. Call it to create a new admin user.")
	# 				exit()
	# 	  		# Redirect
	...
	# 			print("[+]The remote host tried to redirect to: %s" % r.headers['location'])
	# 			self.url = r.headers['location']
	# 		else:
	#   			# print critical("Redirection not followed - End of the scan !")
	# 			print("[!]Error! Redirection not followed - End of the scan !")
	# 			exit()
	...
	"""
	def is_readme(self):
		r = requests.get(self.url + 'readme.html', headers={"User-Agent":self.agent}, verify=False, timeout=10)
		...
		if "200" in str(r):
			...
			if len(matches) > 0 and matches[0] is not None:
				if matches[0] != "":
					...
					print("[+]The Wordpress '%s' file exposing a version number: %s" % (self.url+'readme.html', matches[0]))
	...
	def is_debug_log(self):
		r = requests.get(self.url + 'debug.log', headers={"User-Agent":self.agent}, verify=False, timeout=10)
	...
	def is_backup_file(self):
		...
		for b in backup:
			r = requests.get(self.url + b, headers={"User-Agent":self.agent}, verify=False, timeout=10)
			if "200" in str(r) and "404" not in r.text :
				# self.files.add(b)
				# print critical("A wp-config.php backup file has been found in: %s" % (self.url + b) )
				print("[+]Error! A wp-config.php backup file has been found in: %s" % (self.url + b))
	...
	"""
	def is_xml_rpc(self):
		r = requests.get(self.url + "xmlrpc.php", headers={"User-Agent":self.agent}, verify=False, timeout=10)
	...
	def is_directory_listing(self):
		...
		for directory, name in zip(directories,dir_name):
			r = requests.get(self.url + directory, headers={"User-Agent":self.agent}, verify=False, timeout=10)
			if "Index of" in r.text:
				...
				print("[+]%s directory has directory listing enabled : %s" % (name, self.url + directory))
	...
	def is_robots_text(self):
		r = requests.get(self.url + "robots.txt", headers={"User-Agent":self.agent}, verify=False, timeout=10)
		if "200" in str(r) and "404" not in r.text :
			# self.files.add("robots.txt")
			# print info("robots.txt available under: %s " % (self.url+"robots.txt") )
			print("[+]robots.txt available under: %s " % (self.url+"robots.txt"))
			lines = r.text.split('\n')
			for l in lines:
				if "Disallow:" in l:
	...
	def is_common_file(self):
		...
		for f in files:
			r = requests.get(self.url + f, headers={"User-Agent":self.agent}, verify=False, timeout=10)
			if "200" in str(r) and "404" not in r.text :
				# self.files.add(f)
				# print info("%s available under: %s " % (f, self.url+f) )
				print("[+]%s available under: %s " % (f, self.url+f))
	...
	def full_path_disclosure(self):
		text = requests.get(self.url + "wp-includes/rss-functions.php", headers={"User-Agent":self.agent}, verify=False, timeout=10).text
		...
		if matches != []:
			...
			print("[+]Full Path Disclosure (FPD) in %s exposing %s" % (self.url + "wp-includes/rss-functions.php", matches[0].replace('\n','')))
	...
	def enum_wordpress_users(self):
		r = requests.get(self.url + "wp-json/wp/v2/users", headers={"User-Agent":self.agent} , verify=False, timeout=10)
		...
		if "200" in str(r):
			...
			for user in users:
				...
				print("[+]\tIdentified the following user : %s, %s, %s" % (user['id'], user['name'], user['slug']))
			self.users = users

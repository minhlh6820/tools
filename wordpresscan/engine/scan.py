...
class Scan_Engine:
	def __init__(self, wordpress, aggressive):
		...
		self.wp_vuln_id = self.list_wp_version_vulnerabilities(wordpress)

		if aggressive == False:
			self.theme_vuln_id = self.enumerating_themes_passive(wordpress)
			self.plugin_vuln_id = self.enumerating_plugins_passive(wordpress)
		else:
	...
	def fingerprint_wp_version_meta_based(self, wordpress):
		...
		if match != []:
			...
			print("[+]WordPress version %s identified from advanced fingerprinting" % wordpress.version)
			return True
		return False
	...
	"""
	def list_wp_version_vulnerabilities(self, wordpress):
		# Load json file
		wp_json_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), "../database/wordpresses.json")
		with open(wp_json_file) as data_file:
		...
		if wordpress.version not in data:
			...
			print("[+]The version %s isn't in the database - Please try the option --update" % (wordpress.version))
			return
		...
		for vuln in data[version]["vulnerabilities"]:
			...
			print("[+]\t%s : %s - ID:%s" % (vuln['vuln_type'], vuln['title'] , vuln['id']))
			# print info("\tFixed in %s"% vuln['fixed_in'])
			print("[+]\tFixed in %s"% vuln['fixed_in'])
	...
	def enumerating_themes_passive(self, wordpress):
		...
		print("[+]Enumerating themes from passive detection ...")
		...
		for m in match:
			...
			if m[0] not in theme.keys():
				theme[m[0]] = m[1]
				list_id += display_vulnerable_component(theme_name, theme_version, "themes")
	...
	def enumerating_plugins_passive(self, wordpress):
		...
		for m in match:
			...
			if plugin_name not in plugin.keys() and m[1]!='1':
				...
				list_id += display_vulnerable_component(plugin_name, plugin_version, "plugins")
	...
	def enumerating_themes_aggressive(self, wordpress):
		...
		print("[+]Enumerating themes from aggressive detection ...")
	...
	def enumerating_plugins_aggressive(self, wordpress):
		...
		print("[+]Enumerating plugins from aggressive detection ...")
